...
export const POST = async (req: NextRequest) => {
  ...
  try {
    ...
    console.log("[AI Match API] Received model:", JSON.stringify(selectedModel));
    ...
    switch (selectedModel.provider) {
      case AiProvider.OPENAI:
        console.log("[AI Match API] Using OpenAI with model:", selectedModel.model);
        response = await getJobMatchByOpenAi(resume, job, selectedModel.model);
        break;
      case AiProvider.OLLAMA:
        console.log("[AI Match API] Using Ollama with model:", selectedModel.model);
        response = await getJobMatchByOllama(resume, job, selectedModel.model);
        break;
      default:
        console.log("[AI Match API] Unknown provider, defaulting to OpenAI");
        response = await getJobMatchByOpenAi(resume, job, selectedModel.model);
        break;
    }
